...
export default function ERC20HoldingsForm() {
    ...
    const [erc20Holdings, updateERC20Holdings] = useState<{ holdings: ERC20HoldingsType }>();
    const [erc20Transfers, updateERC20Transfers] = useState<{ transfers: ERC20TransfersType }>();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!addressValidator(walletAddressRef.current!.value.trim())){
            ...
            const erc20HoldingsData = await fetch('/api/optimism-erc20-holdings', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ walletAddress: walletAddressRef.current!.value.trim() })
            });

            const erc20TransfersData = await fetch('/api/optimism-erc20-transfers', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ walletAddress: walletAddressRef.current!.value.trim() })
            });
            ...
            if (erc20TransfersData.ok) {
                ...
                updateERC20Transfers(erc20TransfersResponse);
            }
            else {
                throw new Error();
            }
    ...
    return (
        <>
            ...
            { erc20Holdings ? <ERC20HoldingsInfoTable data={erc20Holdings.holdings} /> : null }
            { erc20Transfers ? <ERC20TransfersInfoTable data={erc20Transfers.transfers} address={walletAddressRef.current!.value.trim()} /> : null }
        </>
    )
}